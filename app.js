...
(() => {
  ...
  /* ========== Vétusté globale : mini-dashboard ========== */
  const VETUSTE_YEARS = ['2022', '2023', '2024', '2025', '2026'];
  const VETUSTE_BUCKETS = [
  ...
  const buildVetusteDataset = (buildingsMap = {}) => {
    const entries = Object.entries(buildingsMap);
    return entries.map(([id, building]) => {
      const seed = hashString(`${id}-${building.label}-${building.sre}`);
      const rng = createRng(seed);
      const base = 25 + Math.round(rng() * 50);
      const slope = -5 + (rng() * 10);

      const yearly = VETUSTE_YEARS.map((year, idx) => {
        const progress = idx / (VETUSTE_YEARS.length - 1);
        const drift = slope * progress;
        const noise = (rng() - 0.5) * 6;
        const score = clampValue(base + drift + noise, 5, 95);
        return { year, score: Math.round(score) };
      });

      const components = {
        structure: Math.round(clampValue(base + (rng() - 0.5) * 18, 5, 95)),
        enveloppe: Math.round(clampValue(base + (rng() - 0.5) * 18, 5, 95)),
        cvc: Math.round(clampValue(base + (rng() - 0.5) * 18, 5, 95)),
        electrique: Math.round(clampValue(base + (rng() - 0.5) * 18, 5, 95)),
      };
      ...
      return {
        ...
        vetuste: {
          yearly,
          components,
          latest: yearly[yearly.length - 1]?.score || base,
        },
      };
    });
  };

  function setupVetusteDashboard() {
    const panel = $('#panel-etat-vetuste');
    if (!panel) return;

    const buildingListEl = panel.querySelector('[data-vetuste-building-list]');
    const searchInput = panel.querySelector('[data-vetuste-search]');
    const sreSlider = panel.querySelector('[data-vetuste-sre]');
    const sreOutput = panel.querySelector('[data-vetuste-sre-output]');
    const countEl = panel.querySelector('[data-vetuste-count]');
    const barsWrap = panel.querySelector('[data-vetuste-distribution] .vetuste-bars');
    const emptyEl = panel.querySelector('[data-vetuste-empty]');
    const trendAvg = panel.querySelector('[data-vetuste-trend-avg]');
    const trendSel = panel.querySelector('[data-vetuste-trend-sel]');
    const axisEl = panel.querySelector('[data-vetuste-axis]');
    const compWrap = panel.querySelector('[data-vetuste-components]');
    const selectionLabel = panel.querySelector('[data-vetuste-selection-label]');

    const baseData = buildVetusteDataset(window.STRATOS_BUILDINGS?.buildings || {});
    ...
    const filteredBuildings = () => {
      ...
      return baseData.filter((b) => {
        const matchesSearch = !term
          ...
          || (b.typology || '').toLowerCase().includes(term);
        const matchesSre = Number(b.sre) >= state.sreMin;
        return matchesSearch && matchesSre;
      });
    };

    const updateBuildingList = (buildings) => {
      ...
      buildings.forEach((building) => {
        ...
        item.type = 'button';
        item.className = 'vetuste-building';
        item.dataset.id = building.id;
        item.setAttribute('role', 'listitem');
        if (building.id === state.selection) item.classList.add('is-selected');

        const title = document.createElement('div');
        title.className = 'vetuste-building__title';
        title.textContent = building.label;

        const meta = document.createElement('div');
        meta.className = 'vetuste-building__meta';
        meta.textContent = `${building.typology || 'Typologie inconnue'} · ${NF.format(Math.round(building.sre))} m²`;

        const score = document.createElement('div');
        score.className = 'vetuste-building__score';
        score.textContent = `IVG ${formatScore(building.vetuste.latest)}`;

        item.append(title, meta, score);
        item.addEventListener('click', () => {
          state.selection = state.selection === building.id ? null : building.id;
          update();
        });
        buildingListEl.append(item);
      });
    };

    const updateDistribution = (buildings) => {
      if (!barsWrap) return;
      barsWrap.innerHTML = '';
      const selectionBuilding = buildings.find(b => b.id === state.selection) || null;
      const buckets = VETUSTE_BUCKETS.map((bucket) => {
        const count = buildings.reduce((acc, building) => {
          const value = building.vetuste.latest;
          if (value >= bucket.min && value < bucket.max) return acc + 1;
          return acc;
        }, 0);
        return { ...bucket, count };
      });

      const maxCount = buckets.reduce((acc, bucket) => Math.max(acc, bucket.count), 0) || 1;
      buckets.forEach((bucket) => {
        const bar = document.createElement('div');
        bar.className = 'vetuste-bar';
        bar.dataset.bucket = bucket.key;

        const fill = document.createElement('div');
        fill.className = 'vetuste-bar__fill';
        const value = document.createElement('div');
        value.className = 'vetuste-bar__value';
        const height = Math.round((bucket.count / maxCount) * 100);
        value.style.setProperty('--height', `${height}%`);
        fill.append(value);

        const label = document.createElement('div');
        label.className = 'vetuste-bar__label';
        label.textContent = bucket.label;

        const count = document.createElement('div');
        count.className = 'vetuste-bar__count';
        count.textContent = `${bucket.count} bât.`;

        if (selectionBuilding) {
          const val = selectionBuilding.vetuste.latest;
          if (val >= bucket.min && val < bucket.max) {
            const badge = document.createElement('div');
            badge.className = 'vetuste-bar__badge';
            badge.textContent = 'Sélection';
            bar.append(badge);
          }
        }
      ...
      if (countEl) countEl.textContent = `${buildings.length} bât.`;
      if (emptyEl) emptyEl.hidden = buildings.length > 0;
    };
    ...
    const updateTrend = (buildings) => {
      if (!trendAvg || !trendSel || !axisEl) return;
      const averages = VETUSTE_YEARS.map((year) => {
        const scores = buildings.map(b => b.vetuste.yearly.find(y => y.year === year)?.score).filter(Number.isFinite);
        if (!scores.length) return null;
        const avg = scores.reduce((sum, val) => sum + val, 0) / scores.length;
        return Math.round(avg);
      });

      const avgValues = averages.map(v => (Number.isFinite(v) ? v : 0));
      trendAvg.setAttribute('points', toPolyline(avgValues));

      const selected = state.selection
        ? buildings.find(b => b.id === state.selection)
          || baseData.find(b => b.id === state.selection)
        : null;
      if (selected) {
        const selValues = VETUSTE_YEARS.map(year => selected.vetuste.yearly.find(y => y.year === year)?.score || 0);
        trendSel.setAttribute('points', toPolyline(selValues));
        trendSel.parentElement?.classList.add('has-selection');
      } else {
        trendSel.setAttribute('points', '');
        trendSel.parentElement?.classList.remove('has-selection');
      }

      axisEl.innerHTML = '';
      VETUSTE_YEARS.forEach((year) => {
    ...
    const updateComponents = (buildings) => {
      if (!compWrap) return;
      const selection = state.selection
        ? buildings.find(b => b.id === state.selection)
          || baseData.find(b => b.id === state.selection)
        : null;
      const components = ['structure', 'enveloppe', 'cvc', 'electrique'];

      components.forEach((component) => {
        const row = compWrap.querySelector(`[data-component="${component}"]`);
        const valueEl = row?.querySelector('[data-component-value]');
        if (!row || !valueEl) return;

        let score;
        if (selection) {
          score = selection.vetuste.components[component];
        } else {
          const scores = buildings.map(b => b.vetuste.components[component]).filter(Number.isFinite);
          if (!scores.length) score = 0;
          else score = Math.round(scores.reduce((sum, val) => sum + val, 0) / scores.length);
        }

        row.style.setProperty('--width', `${clampValue(score, 0, 100)}%`);
        valueEl.textContent = formatScore(score);
      });

      if (selectionLabel) {
        selectionLabel.textContent = selection ? selection.label : 'Parc filtré';
      }
    };
    ...
    const update = () => {
      ...
      updateDistribution(buildings);
      updateTrend(buildings);
      updateComponents(buildings);
    };
    ...
    if (sreSlider && sreOutput) {
      const updateSre = () => {
        state.sreMin = Number(sreSlider.value) || 0;
        sreOutput.textContent = `${NF.format(state.sreMin)} m²`;
        update();
      };
      sreSlider.addEventListener('input', updateSre);
      updateSre();
    }
